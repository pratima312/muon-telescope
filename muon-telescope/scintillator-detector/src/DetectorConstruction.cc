...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4LogicalVolume* logicContainer = new G4LogicalVolume(solidContainer, worldMat, "Container_LV");
    ...
    fLayer = new Layer();
    fLayer->Place(logicContainer, G4ThreeVector(0,0,0), nullptr);

    new G4PVPlacement(nullptr, {}, logicContainer, "Detector Whole", logicWorld, false, 0 ,true);

    auto* lvStore = G4LogicalVolumeStore::GetInstance();
    for (auto& lv : *lvStore)
    {
        if (lv->GetName() == "TriangularBarLV")
        {
            fTriangularBarLV = lv;
            break;
        }
    ...
    if (!fTriangularBarLV)
    {
        G4Exception("DetectorConstruction::Construct()",
                    ...
                    "Could not find TriangularBarLV logical volume.");
    }
...
void DetectorConstruction::ConstructSDandField()
{
  auto* sd = new TriangularBarSD("TriangularBarSD", "TriangularBarHitsCollection");
  G4SDManager::GetSDMpointer()->AddNewDetector(sd);
  SetSensitiveDetector("TriangularBarLV", sd, true);
}

 G4LogicalVolume* DetectorConstruction::GetTriangularBarLV() const
{
    return fTriangularBarLV;
}  